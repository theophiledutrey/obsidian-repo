...
Ok top, j'aimerai que tu écrive la suite du rapport. Je vais te donner tous les éléments.

On est ensuite passé sur l'Active directory. La premiere question était sur qu est ce que l'ADCS. Je conaissais que le concept de l'ADCS sur la gestion des certificats mais je n ai pas su répondre précisement à la question. Après quelque recherche de mon côté j ai compris de quoi il s agit (tu expliras). Ensuite on est passé sur kerbreos. J'ai su expliqué que kerberos est un process d'authentification sur l'active directectory. Au lieu de se connecter directement à un service l'utilisateur passe par le KDC (composé de deux services: l'AS et le TGS) pour obtenir un ticket de session qui lui donneras l acces au service pour un certain temps. Pour cela il y a 4 étapes utilisé. AS_REQ qui consiste à indiqué son nom à l'AS et demande une pre auth si elle est activé. AS_REP, l'AS envoie un TGT chiffré avec la clé du kdc au client et il lui envoi aussi une clé de session chiffré avec le mdp dérivé de l'utilisateur. L'utilisateur est donc capable de déchiffrer cette clé. TGS_REQL: l'user envoie son token chiffre au TGS ainsi qu'un authenticator contenant identifiant client + timestamp) et qu'il chiffre avec la clé de session recu à l'étape précédente. Le TGS recoit ça déchiffre le TGT avec sa clé. Le tgt contient la clé de session qui a permi de chiffrer l'authentificator. Il s en sert donc pour le déchiffrer, et vérifie fraîcheur/identité. Si le donné sont cohérentés on passe à la derniere étape: le TGS renvoie une **TGS-REP** contenant un **service ticket** chiffré avec la clé du service ciblé et une copie de la clé de session pour le client.
